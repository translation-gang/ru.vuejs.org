...
test('Foo', () => {
  ...
  // проверяем, что ошибка отрисовалась
...
[Vue Test Utils](https://github.com/vuejs/vue-test-utils) — официальная библиотека для модульного тестирования Vue-компонентов. Шаблон `webpack` [vue-cli](https://github.com/vuejs/vue-cli) идёт в комплекте с Karma или Jest, как с хорошо поддерживаемыми программами запуска тестов, и есть некоторые [руководства](https://vue-test-utils.vuejs.org/guides/) в документации Vue Test Utils.
...
Тесты снимками (snapshot) сохраняют разметку вашего Vue-компонента и сравнивают с новой, создаваемой при каждом выполнении теста. Если что-то изменяется, разработчик будет в курсе и сможет решить, было ли решение намеренным (компонент был обновлён) или случайным (компонент работает некорректно).
...
Тесты высокого уровня, такие как end-to-end тесты, работают намного медленнее. Обычно их запускают перед публикацией для гарантии, что каждая часть системы работает корректно.
