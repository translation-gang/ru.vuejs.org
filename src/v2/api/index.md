...
- **Использование:**
  ...
  Определение пользовательских алиасов для директивы `v-on`.
...
- **Использование:**
  ...
  > Добавлено в версии 2.1.0: если коллбэк не указан, а окружение поддерживает промисы — возвращает промис.
...
- **Пример:**
  ...
  <p class="tip">Обратите внимание, что __вам не стоит использовать стрелочные функции в параметре `data`__ (напр. `data: () => { return { a: this.myProp }}`). Поскольку стрелочные функции связываются с родительским контекстом, `this` не будет указывать на инстанс Vue и `this.myProp` окажется неопределенным.</p>
...
- **Подробности:**
  ...
  <p class="tip">Обратите внимание, что __вам не стоит использовать стрелочные функции при определении вычисляемых свойств__ (напр. `aDouble: () => this.a * 2`). Поскольку стрелочные функции связываются с родительским контекстом, `this` не будет указывать на инстанс Vue, и `this.a` окажется неопределенным.</p>
...
- **Подробности:**
  ...
  <p class="tip">Обратите внимание, что __вам не стоит использовать стрелочные функции при определении методов__ (напр. `plus: () => this.a++`). Поскольку стрелочные функции связываются с родительским контекстом, `this` не будет указывать на инстанс Vue, и `this.a` окажется неопределенным.</p>
...
Хук (hook - буквально "крючок"), функция, вызывающаяся в определенный момент жизненного цикла сущности, например, непосредственно перед или после создания компонента Vue. Хук позволяет изменить стандартное поведение на пользовательское.
...
- **Подробности:**
  ...
  Вызывается при изменении данных, до повторного рендера и обновления виртуального DOM-а.

  В этом хуке можно дополнительно изменять состояние, и это не вызовет нового повторного рендера.
...
- **Подробности:**
  ...
  Кроме того, компоненты с указанным `name` проще отлаживать из-за более понятных сообщений в консоли. В [vue-devtools](https://github.com/vuejs/vue-devtools) все безымянные компоненты крайне неинформативно называются `<AnonymousComponent>`. При  задании `name` дерево компонентов становится намного информативнее.
...
- **Использование:**
  ...
  Создает подписку на пользовательское событие текущего vm. Такое событие можно выбросить функцией `vm.$emit`. Коллбэк получит все дополнительные аргументы, переданные этому методу.
...
- **Аргументы:**
  ...
  Выбрасывает событие в текущем инстансе. Все дополнительно указанные параметры будут переданы в коллбэк подписки.
...
- **Использование:**
  ...
  Заставляет инстанс Vue произвести повторный рендер. Обратите внимание, что этот метод затронет не все дочерние компоненты, а только сам инстанс и дочерние компоненты с контентными слотами.
...
- **Подробности:**
  ...
  <p class="tip">Динамический рендеринг произвольного HTML-кода на сайте — это очень опасная практика, так как легко приводит к [XSS-атакам](https://en.wikipedia.org/wiki/Cross-site_scripting). Передавайте в `v-html` только содержимое, которому можно довреять, и **никогда** не передавайте сюда ничего предоставленного пользователями.</p>
...
- **Использование:**
  ...
  Осуществляет рендеринг элемента, только если передавемое выражение истинно. При изменении значения на противоположное, элемент и содержащиеся в нём компоненты с директивами уничтожаются/пересоздаются. Если элемент — `<template>`, вместо элемента будет отрендерено его содержимое. Это нужно, чтобы завернуть в один v-if несколько элементов сразу.
...
- **Использование:**
  ...
  **Однократно** рендерит элемент или компонент. При повторном рендере он, а также все его потомки, рассматриваются как статический контент и пропускаются. Это поможет увеличить производительность обновлений.
...
- **Использование:**
  ...
  Оберните динамические компоненты тегом `<keep-alive>`, чтобы кешировать инстансы интерактивных компонентов вместо того, чтобы их уничтожать. Так же, как и `<transition>`, `<keep-alive>` — абстрактный компонент: при рендере он не превращается в элемент DOM, и не показывается в цепочке родителей компонента.
