...
- **Использование:**
  ...
  Добавляет свойство к реактивному объекту, гарантируя, что новое свойство также будет реактивным и вызовут обновление представления. Это необходимо использовать для добавления новых свойств к реактивным объектам, поскольку Vue не может иначе отслеживать эти изменения (например, `this.myObject.newProperty = 'hi'`).
...
- **Пример:**
  ...
  Обратите внимание, если вы используете стрелочную функцию в свойстве `data`, то `this` не будет экземпляром компонента, но вы можете получить доступ к экземпляру из первого аргумента функции:
...
- **Подробности:**
  ...
  Обратите внимание, если вы используете стрелочную функцию в вычисляемом свойстве, то `this` не будет экземпляром компонента, но вы можете получить доступ к экземпляру из первого аргумента функции:
