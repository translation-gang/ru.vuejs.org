...
- **Принимает:** JavaScript выражение, допустимое в позиции аргумента (можно использовать деструктуризацию в [поддерживаемых окружениях](../guide/components-slots.html#Slot-Props-Destructuring)). Опционально — требуется только в случае, когда используются входные параметры у слота.

- **Параметр:** имя слота (опционально, по умолчанию `default`)

- **Ограничение:**
  - `<template>`
  - [компоненты](../guide/components-slots.html#Abbreviated-Syntax-for-Lone-Default-Slots) (для одиночного слота по умолчанию с входными параметрами)
