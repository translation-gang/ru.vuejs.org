...
- **Входные параметры:**
  ...
  - `duration` - number | { enter: number, leave: number }, указывает длительность перехода, по умолчанию анимация завершается по первому событию transitionend|animationend.
  - `enter-class` — string
  - `leave-class` — string
  - `appear-class` — string
