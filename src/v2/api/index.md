...
- **Использование:**
  ...
  Определение обработчика для ошибок, не отловленных во время рендеринга компонентов и в наблюдателях. Обработчик получает аргументами ошибку и действующий экземпляр Vue.
...
- **Использование:**
  ...
  Установка `true` включает отслеживание производительности на этапах инициализации, компиляции, рендера и обновления компонента на графике в инструментах разработчика браузера. Работает только в режиме разработки в браузерах, которые поддерживают [performance.mark](https://developer.mozilla.org/en-US/docs/Web/API/Performance/mark) API.
...
- **См. также:** [Входные параметры](../guide/components.html#Передача-данных-в-дочерние-компоненты-через-входные-параметры)
...
- **Подробности:**
  ...
  **Не вызывается при рендеринге на стороне сервера.**
...
- **Подробности:**
  ...
  Обратите внимание, что `mounted` **не гарантирует** что все дочерние компоненты будут уже примонтированы. Если необходимо подождать пока не будут отрендерены все дочерние, можно воспользоваться [vm.$nextTick](#vm-nextTick) внутри `mounted`:
...
- **Подробности:**
  ...
  **Не вызывается при рендеринге на стороне сервера, потому что на сервере выполняется только начальный рендеринг.**
...
- **Подробности:**
  ...
  Обратите внимание, что `updated` **не гарантирует** что все дочерние компоненты будут уже перерендерены. Если необходимо подождать пока все не будут перерендерены, можно воспользоваться [vm.$nextTick](#vm-nextTick) внутри `updated`:
...
- **См. также:**
  ...
  - [Динамические & Асинхронные компоненты — keep-alive с динамическими компонентами](../guide/components-dynamic-async.html#keep-alive-с-динамическими-компонентами)
...
- **См. также:**
  - [Доступ к экземплярам дочерних компонентов & элементов](../guide/components-edge-cases.html#Доступ-к-экземплярам-дочерних-компонентов-amp-элементов)
...
- **См. также:** [Динамические & Асинхронные компоненты — keep-alive с динамическими компонентами](../guide/components-dynamic-async.html#keep-alive-с-динамическими-компонентами)
