...
## Опции — обработчики жизненного цикла

Контекст `this` обработчиков указывает на инстанс Vue c данными, вычисляемыми свойствами и методами. Это значит, что __вам не следует использовать стрелочные функции для определения обработчиков жизненного цикла__ (напр. `created: () => this.fetchTodos()`). Поскольку стрелочные функции связываются с родительским контекстом, `this` не будет указывать на инстанс Vue, и `this.fetchTodos` окажется неопределенным.
...
- **Подробности:**
  ...
  **При рендере на стороне сервера этот обработчик не вызывается.**
...
- **Подробности:**
  ...
  В этом обработчике можно дополнительно изменять состояние, и это не вызовет нового повторного рендера.
...
- **Подробности:**
  ...
  DOM компонента уже будет обновлён к моменту вызова этого обработчика, поэтому можно выполнять зависящие от DOM операции. Старайтесь избегать изменения состояния в этом обработчике, чтобы не попасть в бесконечный цикл обновления.
...
- **Подробности:**
  ...
  Вызывается после уничтожения инстанса Vue. К моменту вызова этого обработчика, все директивы инстанса Vue уже отвязаны, все подписчики событий удалены, а все дочерние инстансы Vue уничтожены.
...
- **Подробности:**
  ...
  Массив объектов-миксинов. Миксины могут содержать те же опции, что и обычные объекты инстансов Vue — они будут добавлены к основным опциям согласно логике `Vue.extend()`. Например, если и миксин и объект оба содержат обработчик `created`, вызваны будут они оба.

  Обработчики миксинов вызываются в порядке их определения, до вызова собственных обработчиков компонента.
...
- **Использование:**
  ...
  Вызывает обработчики `beforeDestroy` и `destroyed`.
...
- **Принимает:** `string`
  ...
  - Быть уверенны, что все обработчики жизненного цикла компонента будут вызваны
...
- **Использование:**
  ...
  Когда компонент, расположенный внутри `<keep-alive>` показывается или скрывается, вызываются его обработчики жизненного цикла `activated` или `deactivated` соответственно.
