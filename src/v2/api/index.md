...
- **Использование:**
  ...
  Vue.config.errorHandler = function (err, vm, type) {
    // обработка ошибки
    // `type` это тип Vue-специфичной ошибки, например в каком хуке жизненного цикла
...
- **Подробности:**
  ...
  Эта пара опций используется вместе, чтобы позволить компоненту-родителю внедрять зависимости во всех его компонентах-потомках, независимо от того, насколько глубоко в иерархии они находятся. Если вы знакомы с React, это очень похоже на функцию контекста.
  ...
  Компонент-провайдер обязательно должен быть в цепочке родителей компонента, желающего внедрить предоставленные им свойства.
