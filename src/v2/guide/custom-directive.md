...
Для жизненного цикла директивы можно указать следующие хуки (все — необязательные):
...
- **binding**: Объект, содержащий следующие свойства:
  ...
  - **oldValue**: Предыдущее переданное в директиву значение. Доступно только для хуков `update` и `componentUpdated` и передаётся независимо от того, произошло ли в действительности изменение значения.
...
Зачастую может оказаться, что используются только хуки `bind` и `update`, причём с одним и тем же функционалом. В таком случае можно сократить запись, напрямую передав функцию:
