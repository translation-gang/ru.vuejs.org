...
Для жизненного цикла директивы можно указать следующие хуки (все они, разумеется, необязательны):
...
Зачастую может оказаться, что используются только хуки `bind` и `update`, причём с одним и тем же функционалом. В таком случае можно сократить запись, передав функцию напрямую:
