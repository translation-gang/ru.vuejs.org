...
> До версии 2.5.0 `slot-scope` именовался `scope` и мог использоваться только с элементом `<template>`. С версии 2.5.0 `slot-scope` не имеет этого ограничения и может использоваться на любом элементе или компоненте в слоте.
