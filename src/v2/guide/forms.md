...
<p class="tip" id="vmodel-ime-tip">Для языков, требующих [IME](https://en.wikipedia.org/wiki/Input_method) (китайский, японский, корейский и т.д.), можно заметить, что `v-model` не обновляется по мере IME-композиции. Если вы хотите обрабатывать и эти обвновления, используйте события `input`.</p>
