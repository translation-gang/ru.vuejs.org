...
Вы можете использовать директиву `v-model` для двунаправленного связывания с элементами форм input и textarea. Корректный способ обновления элемента выбирается автоматически в зависимости от типа элемента. Несмотря на слегка волшебный вид, `v-model` в действительности представляет собой всего лишь синтаксический сахар для обновления данных в элементах ввода данных пользователем, с некоторыми поправками для краевых случаев.
...
<p class="tip" id="vmodel-ime-tip">Для языков, требующих [IME](https://en.wikipedia.org/wiki/Input_method) (китайский, японский, корейский и т.д.), можно заметить6 что `v-model` не обновляется по мере IME-композиции. Если вы хотите обрабатывать и эти обвновления, используйте события `input`.</p>
...
Для радиокнопок, чекбоксов и выпадающих списков выбора, в качестве параметров `v-model` обычно указываются статические строки (для чекбоксов — булевы значения):
...
Зачастую это оказывается полезным, так как даже при указанном атрибуте `type="number"` значением HTML input'а всегда является строка.
