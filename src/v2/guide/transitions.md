...
3. `v-enter-to`: **Доступно только в&nbsp;версиях >=2.1.8.** Означает, что анимация появления элемента выполняется. Класс добавляется в&nbsp;следующем фрейме после вставки элемента (тогда&nbsp;же, когда будет удалён `v-enter` ), удаляется после завершения перехода или анимации.
...
6. `v-leave-to`: **Доступно только в&nbsp;версиях >=2.1.8.** Означает, что анимация исчезновения элемента выполняется. Класс добавляется в&nbsp;следующем фрейме после начала анимации исчезновения (тогда&nbsp;же, когда будет удалён `v-leave`), а&nbsp;удаляется после завершения перехода или анимации.
...
Одним из&nbsp;самых распространённых типов анимации&nbsp;&mdash; это CSS-переходы. Вот небольшой пример:
...
Подробно [переходы между компонентами](#Переходы-между-компонентами) мы&nbsp;обсудим позднее, а&nbsp;сейчас поговорим о&nbsp;переходах с&nbsp;помощью `v-if`/`v-else`. Один из&nbsp;наиболее частых случаев&nbsp;&mdash; переход от&nbsp;списка к&nbsp;сообщению, что список пуст:
...
Во&nbsp;время перехода от&nbsp;кнопки &laquo;on&raquo; к&nbsp;кнопке &laquo;off&raquo; одновременно отображаются обе кнопки: одна&nbsp;&mdash; исчезая, другая&nbsp;&mdash; появляясь. Так `<transition>` ведёт себя по&nbsp;умолчанию.
...
Тем не&nbsp;менее, одновременное сокрытие и&nbsp;появление элементов&nbsp;&mdash; это не&nbsp;всегда&nbsp;то, чего хочется. Поэтому Vue предоставляет альтернативные **режимы перехода**:
...
Изменив всего лишь один атрибут, мы&nbsp;&laquo;починили&raquo; анимацию перехода, не&nbsp;прибегая к&nbsp;редактированию стилей.
...
Переходы между компонентами&nbsp;&mdash; ещё проще, нам даже не&nbsp;нужен атрибут `key`. Всё, что нужно&nbsp;&mdash; завернуть [динамический компонент](components.html#Динамическое-переключение-компонентов) в&nbsp;`<transition>`:
...
Хотя это и&nbsp;может выглядеть как магия, &laquo;под капотом&raquo; Vue использует довольно простую анимационную технику под названием [FLIP](https://aerotwist.com/blog/flip-your-animations/), которая позволяет плавно перевести элементы с&nbsp;их&nbsp;старых позиций на&nbsp;новые, используя CSS-трансформации.
...
Анимация переходов может быть переиспользована благодаря компонентной системе Vue. Всё, что необходимо сделать&nbsp;&mdash; это поместить компонент `<transition>` или `<transition-group>` в&nbsp;корне компонента, а&nbsp;затем передать в&nbsp;этот компонент потомков.
...
Да, даже анимационные переходы во&nbsp;Vue управляются данными! Простейший пример&nbsp;&mdash; связывание атрибута `name` с&nbsp;динамическим свойством.
...
На&nbsp;самом деле, любой атрибут может быть динамически связан. И&nbsp;речь не&nbsp;только об&nbsp;атрибутах. Поскольку хуки&nbsp;&mdash; это просто методы, у&nbsp;них есть доступ ко&nbsp;всем данным в&nbsp;текущем контексте, а&nbsp;значит и&nbsp;JavaScript-анимации могут зависеть от&nbsp;состояния компонента.
...
Наконец, ещё больше возможностей для создания динамических переходов появляется, если создать компоненты, принимающих входные параметры, определяющие природу используемых переходов. Звучит избито, но&nbsp;всё равно&nbsp;&mdash; здесь всё ограничено только вашим воображением.
