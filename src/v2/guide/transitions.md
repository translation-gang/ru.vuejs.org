...
<p class="tip">Для чисто JavaScript'овых переходов, **коллбэки `done` в хуках `enter` и `leave` вызывать обязательно**. В противном случае, они будут вызванны синхронно, что приведёт к досрочному завершению перехода.</p>
