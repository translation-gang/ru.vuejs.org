...
Рекомендуем этот вариант: на [https://cdn.jsdelivr.net/npm/vue](https://cdn.jsdelivr.net/npm/vue) всегда доступна самая свежая версию Vue, загруженная в NPM. Исходный код самого NPM-пакета также доступен по ссылке [https://cdn.jsdelivr.net/npm/vue](https://cdn.jsdelivr.net/npm/vue).

Vue также доступен на [unpkg](https://unpkg.com/vue) и [cdnjs](https://cdnjs.cloudflare.com/ajax/libs/vue/{{vue_version}}/vue.js) (cdnjs синхронизируется с NPM с некоторой задержкой, из-за чего новейшая версия может быть не всегда доступна).
...
- **[UMD](https://github.com/umdjs/umd)**: UMD-сборки можно использовать сразу в браузере, подключая тегом `<script>`. Файлом по умолчанию в jsDelivr CDN из [https://cdn.jsdelivr.net/npm/vue](https://cdn.jsdelivr.net/npm/vue) будет Runtime + Компилятор UMD-сборка (`vue.js`).
