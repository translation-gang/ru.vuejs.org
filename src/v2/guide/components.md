...
Часто встречается ошибка, когда число передается компоненту напрямую в виде константы:
