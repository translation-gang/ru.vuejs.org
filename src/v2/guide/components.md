...
В этом втором примере важно отметить, что дочерний компонент всё остаётся полностью независимым от всего происходящего снаружи. Он всего лишь уведомляет внешний мир о происходящем с ним, включая данные полезной нагрузки в генераторе событий (event emitter), на случай, если родительскому компоненту это будет интересно.
