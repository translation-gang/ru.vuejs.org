...
Если вы поклонник подхода CSS-in-JSS — многие популярные библиотеки поддерживают Vue (например, [styled-components-vue](https://github.com/styled-components/vue-styled-components) и [vue-emotion](https://github.com/egoist/vue-emotion)). Главным отличием между React и Vue здесь будет то, что по умолчанию стилизация Vue выполняется через знакомые `style` теги в [однофайловых компонентах](single-file-components.html).
...
Наконец, стилизация в однофайловых компонентах Vue очень гибка. С помощью [vue-loader](https://github.com/vuejs/vue-loader), вы можете использовать любой препроцессор, пост-процессор и даже глубокую интеграцию с [CSS-модулями](http://vue-loader.vuejs.org/ru/features/css-modules.html) — всё в элементе `<style>`.
