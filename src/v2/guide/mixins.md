...
Например, объект `data` претерпевает поверхностное слияние (на одно свойство в глубину) с объектом `data` компонента, который имеет приоритет в случаях конфликтов.
