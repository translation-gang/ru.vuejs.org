...
Значение выражения, обрамлённого двойными фигурными скобками, интерпретируется как простой текст, а не как HTML. Если вы хотите вывести HTML, вам понадобится директива `v-html`:
...
<p class="tip">Динамический рендеринг произвольного HTML-кода на вашем сайте может быть крайне опасен, так как он может легко привести к [XSS атакам](https://en.wikipedia.org/wiki/Cross-site_scripting). Используйте интерполяцию HTML только на доверенном, и **никогда** — на пользовательском контенте.</p>

### Атрибуты

Синтаксис двойных фигурных скобок не работает с HTML-атрибутами, вместо него используется [директива v-bind](../api/#v-bind):

``` html
<div v-bind:id="dynamicId"></div>
```

С булевыми атрибутами тоже работает - в случае ложного значения атрибут будет удалён полностью:
...
До сих пор мы связывались только со свойствами в шаблонах по простым ключам. Но Vue.js в действительности поддерживает всю мощь выражений JavaScript при связывании данных:
...
В данном случае `href` является аргументом, говорящим директиве `v-bind` связать атрибут `href` элемента со значением выражения `url`.
