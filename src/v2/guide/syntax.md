...
Для работы Vue компилирует шаблоны в рендер-функции виртуального DOM. В сочетании с системой реактивности Vue умеет определять минимальное количество компонентов для повторного рендеринга и применяет минимальное число манипуляций к DOM при изменении состояния приложения.
...
Атрибуты удаляются если значение `null`, `undefined`, или `false`. Но при значениях `0` и `NaN` будут отображаться.
