...
Чтобы связывать DOM с данными инстанса, Vue использует синтаксис, основанный на HTML. Все шаблоны Vue являются валидным HTML-кодом, который могут распарсить все парсеры и браузеры, следующие спецификации.

Для работы Vue компилирует шаблоны в функции рендера виртуального DOM. При изменении состояния приложения система реактивности определяет минимальный набор компонентов для перерендера, в которых меняется минимальное количество DOM.

Если вы знакомы с концепцией виртуального DOM и предпочитаете использовать ничем не ограниченную мощь JavaScript, вы можете также [писать render-функции самостоятельно](render-function.html) вместо шаблонов, в том числе с помощью JSX.
...
Содержимое вставляется как обыкновенный HTML, а связывание данных игнорируется. Обратите внимание, что вы не можете использовать `v-html` для вложения шаблонов друг в друга, так как движок шаблонов Vue не основывается на строках. Вместо этого лучше использовать компоненты, позволяющие компоновать и переиспользовать элементы UI.
...
С булевыми атрибутами этот метод тоже работает. В случае ложного значения атрибут будет удалён полностью:
...
До сих пор мы связывали данные со свойствами в шаблонах только по простым ключам. Но на самом деле Vue поддерживает всю мощь выражений JavaScript при связывании данных:
...
Функция-фильтр получает первым аргументом значение вычисленного выражения.
...
Некоторые директивы могут принимать "агрумент", указываемый после двоеточия после названия директивы. Например, директива `v-bind`используется для реактивного обновления атрибутов HTML:
...
Префикс `v-` служит как визуальное напоминание для обнаружения Vue-специфичных атрибутов в ваших шаблонах. Это может быть полезно, когда  Vue используется для добавления динамического поведения в уже существующей разметке, но для часто используемых директив может показаться слишком многословным. В то же время, необходимость в `v-` становится куда меньшей при создании [одностраничных приложений](https://ru.wikipedia.org/wiki/Одностраничное_приложение), где Vue контролирует все шаблоны. Поэтому Vue предоставляет специальные сокращения для двух наиболее часто используемых директив, `v-bind` и `v-on`:
