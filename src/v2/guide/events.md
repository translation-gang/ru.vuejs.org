...
``` html
<!-- событие scroll не отменяет стандартного поведения прокрутки -->
<div v-on:scroll.passive="onScroll">...</div>
```

В дополнение к этим модификаторам Vue предоставляет модификатор `.passive` для улучшения производительности, особенно на мобильных устройствах.
Например, при выполнении прокрутки, браузер станет прокручивать страницу только после завершения процесса, потому что браузер не знает, будет ли вызываться `event.preventDefault()` внутри обработчика.
Модификатор `.passive` может быть использован, чтобы сообщить браузеру, что это событие не будет отменять поведения для события по умолчанию.

<p class="tip">Не используйте вместе `.passive` и `.prevent`. Пассивный обработчик не может предотвращать событие по умолчанию.</p>
