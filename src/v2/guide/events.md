...
<!-- т.е. событие, направленное на внутренний элемент, сначала обрабатывается здесь, перед тем, как будет обрабатываться этим элементом -->
...
Модификатор `.exact` должен использоваться в комбинации с другими системными модификаторами, чтобы указать, что для активации обработчика должна быть нажата точная комбинация модификаторов.

``` html
<!-- это сработает даже если Alt или Shift также нажаты -->
<button @click.ctrl="onClick">A</button>

<!-- это сработает только когда нажат Ctrl -->
