...
Теперь эти поля ввода будут рендериться заново при каждом переключении. Смотрите сами:
...
При совместном использовании `v-if` и `v-for`, `v-for` имеет более высокий приоритет. Подробности на странице [рендеринга списков](list.html#v-for-и-v-if).
