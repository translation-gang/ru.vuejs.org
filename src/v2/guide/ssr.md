...
server.get('*', function (request, response) {
  ...
  renderer.renderToString(
    ...
    function (error, html) {
      // Если при пререндеринге произошла ошибка...
      if (error) {
      ...
      // Отсылаем HTML-шаблон, в который вставлен результат пререндеринга приложения
