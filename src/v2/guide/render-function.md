...
Когда браузер читает этот код, он создаёт [дерево «узлов DOM»](https://javascript.info/dom-nodes), чтобы облегчить ему отслеживание всего, как например вы могли бы построить генеалогическое дерево для отслеживания вашей увеличивающейся семьи.
...
Поскольку функциональные компоненты — это просто функции, их рендеринг обходится значительно дешевле. Однако отсутствие постоянного экземпляра означает, что они не будут отображаться в дереве компонентов в [Vue devtools](https://github.com/vuejs/vue-devtools).
