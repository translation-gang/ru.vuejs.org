...
createElement(
  ...
  // {String | Array}
  // Дочерние виртуальные узлы, создаваемые с помощью `createElement()`
...
Все виртуальные узлы в компоненте должны быть уникальными. Это значит, что `render`-функция ниже — не корректна:
...
- `props`: Объект со всеми переданнами входными параметрами
...
Поскольку функциональные компоненты — это просто функции, их отрисовка значительно быстрее. Однако отсутствие постоянного экземпляра означает, что они не будут отображаться в дереве компонентов в браузерном расширении [Vue Devtools](https://github.com/vuejs/vue-devtools).
