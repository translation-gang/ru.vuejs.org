...
Когда они присутствуют на одном элементе, то `v-for` имеет больший приоритет перед `v-if`. Это означает, что `v-if` будет выполняться отдельно для каждой итерации цикла. Это очень полезно при необходимости отобразить только _некоторые_ элементы списка, например:
...
Если нужно чтобы по условию пропускалось выполнение всего цикла, то следует поместить `v-if` на элемент обёртки  (или [`<template>`](conditional.html#Условные-группы-с-использованием-v-if-и-lt-template-gt)). Например:
