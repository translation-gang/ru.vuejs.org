...
В `v-for` можно также передать целое число. В этом случае шаблон повторится указанное количество раз.
