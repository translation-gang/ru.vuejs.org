...
<div id="coin-details" v-if="type && coins[type]">
  <p>{{ coins[type].name }} Адрес:<br>{{ coins[type].address }}</p>
...
Повторяющиеся взносы предоставляют эксклюзивные привилегии, например указание вашего имени в GitHub репозитории Vue, или размещение логотипа вашей компании на этом веб-сайте.
