...
- **Модификаторы:**
  ...
  - `.capture` - добавит слушатель в режиме capture.
...
- **Использование:**
  ...
  На обычном элементе слушает только **нативные события DOM**. На элементе компонента, слушает также **пользовательские события**, вызываемые этим дочерним компонентом.
...
- **Пример:**
  ...
  <!-- слушаем нативное событие в компоненте -->
...
- **См. также:**
  ...
  - [Компоненты - компоненты для вводы данных c пользовательскими событиями](/guide/components.html#Form-Input-Components-using-Custom-Events)
...
- **Использование:**
  ...
  **Однократно** рендерит элемент или компонент. При перерендеринге он, а также все его потомки будут рассматриваться как статический контент и пропускаться. Это поможет увеличить производительность обновлений.
...
- **См. также:**
  ...
  - [Компоненты - дешёвые статические компоненты с v-once](/guide/components.html#Cheap-Static-Components-with-v-once)
